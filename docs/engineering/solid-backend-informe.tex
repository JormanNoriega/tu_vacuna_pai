...
\textbf{127 tests, 0 fallos} (sección~\ref{sec:integracion}).
...
\begin{itemize}
  ...
  \item Smoke boot con Supabase tras la integración: el backend arranca
        (\texttt{Started ApiApplication}), Flyway aplica V11--V15 (schema en
        v15) y el health responde \texttt{UP}; el backend se cierra al
...
El entregable de control de cambios (tabla CC-01..CC-06) y el checklist de
...
\begin{enumerate}
  ...
  \item \textbf{Sync}: el \texttt{SyncEngine} (PR~#1) ya se integró; el
        ...
        \texttt{IdempotencyCoordinator} (adaptado a la firma v2). Pendiente:
        auditar \texttt{identity}, \texttt{reports} y los catálogos
        geográficos con las reglas ArchUnit.
\end{enumerate}
