...
public class ArchitectureTest {
    ...
    @ArchTest
    static final ArchRule controllers_depend_on_services =
        classes().that().resideInAPackage("..controller..")
            .should().dependOnClassesThat().resideInAPackage("..service..")
            .as("los controllers deben delegar en los servicios (ADR-006)");

    @ArchTest
    static final ArchRule controllers_must_return_dtos_not_entities =
        noMethods().that().arePublic()
            .and().areDeclaredInClassesThat().resideInAPackage("..controller..")
            .should().haveRawReturnType(
                DescribedPredicate.describe("a JPA @Entity",
    ...
    @ArchTest
    static final ArchRule repositories_may_depend_on_entities =
        classes().that().resideInAPackage("..repository..")
            .should().dependOnClassesThat().resideInAPackage("..entity..")
            .as("los repositories acceden a las entidades JPA (ADR-006)");

    @ArchTest
    static final ArchRule security_must_not_depend_on_controllers_or_repositories =
