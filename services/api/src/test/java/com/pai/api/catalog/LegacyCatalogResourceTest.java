...
class LegacyCatalogResourceTest {
    private static final Set<String> ALLOWED_TYPES = Set.of("dose", "pneumococcalType",
        "laboratory", "syringe", "dropper", "observation");

    @Test
    void generatedProjectionContainsAllLegacyVaccinesAndMappedOptions() throws Exception {
        try (InputStream input = getClass().getClassLoader()
                .getResourceAsStream("catalog/legacy-vaccines.json")) {
            String json = new String(input.readAllBytes(), StandardCharsets.UTF_8);
            assertThat(json.split("\\\"code\\\"\\s*:", -1)).hasSize(28);
            assertThat(json).contains("COVID-19", "covid19", "todas",
