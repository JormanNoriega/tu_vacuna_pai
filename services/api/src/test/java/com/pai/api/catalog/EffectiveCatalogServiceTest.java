...
class EffectiveCatalogServiceTest {
    ...
    void setUp() {
        ...
        service = new EffectiveCatalogService(
                institutionVaccines, institutionOptions, vaccines, vaccineOptions, identity, new DataScope());
    }
    ...
    private void givenBatchedOptions(
        ...
        when(vaccineOptions.findByVaccineIdInAndActiveTrueOrderByVaccineIdAscSortOrderAscDisplayNameAsc(any()))
                .thenReturn(doses);
    ...
    void list_includesEnabledVaccineWithDosesAndOperationalOptions() {
        ...
        givenBatchedOptions(List.of(dose(vaccineId, "Primera dosis")), List.of(operational(vaccineId, "Pfizer")));

        EffectiveCatalogResponse response = service.list(ACTOR_ID);

        assertThat(response.vaccines()).hasSize(1);
        EffectiveCatalogResponse.EffectiveVaccine effective =
                response.vaccines().get(0);
    ...
    void list_groupsOptionsPerVaccineWhenBatched() {
        ...
        givenBatchedOptions(
                List.of(dose(first, "Influenza 1"), dose(second, "Polio 1")), List.of(operational(first, "Pfizer")));

        EffectiveCatalogResponse response = service.list(ACTOR_ID);

        assertThat(response.vaccines()).hasSize(2);
        EffectiveCatalogResponse.EffectiveVaccine influenza =
                response.vaccines().get(0);
