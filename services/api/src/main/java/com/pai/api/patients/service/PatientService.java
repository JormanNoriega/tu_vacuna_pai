...
public class PatientService {
  ...
  public PatientResponse create(
    ...
    processedOperations.record(
        operationId, "CREATE_PATIENT", patient.getId(), institutionId, request, response);
    return response;
  }
  ...
  public PatientResponse updateDemographics(
    ...
    demographics.save(new PatientDemographicEntity(
        patientId,
        gender,
        blankToNull(request.ethnicity()),
        blankToNull(request.educationLevel()),
        now));
  ...
  private PatientResponse toResponse(PatientEntity patient) {
    ...
    for (PatientContactEntity contact :
    ...
    for (PatientAddressEntity address :
    ...
    for (PatientGuardianEntity guardian :
    ...
    PatientResponse.AffiliationDto affiliationDto = affiliations
        .findByPatientId(patient.getId())
        .map(a -> new PatientResponse.AffiliationDto(
            a.getAffiliationRegime(), a.getInsurer(), a.getInsurerCode()))
        .orElse(null);
    PatientResponse.SpecialConditionsDto specialDto = specialConditions
        .findByPatientId(patient.getId())
        ...
        .orElse(null);
    PatientResponse.UserConditionDto userDto = userConditions
        .findByPatientId(patient.getId())
