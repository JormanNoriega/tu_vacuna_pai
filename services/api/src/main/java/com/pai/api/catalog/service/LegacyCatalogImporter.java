...
public class LegacyCatalogImporter implements ApplicationRunner {
    ...
    public LegacyCatalogImporter(
            VaccineRepository vaccines, VaccineOptionRepository options, VaccineOptionTemplateRepository templates) {
    ...
    public void run(ApplicationArguments args) throws Exception {
        try (InputStream input = new ClassPathResource("catalog/legacy-vaccines.json").getInputStream()) {
            for (JsonNode row : mapper.readTree(input)) {
                ...
                VaccineEntity vaccine = vaccines.findByCode(row.path("code").asText())
