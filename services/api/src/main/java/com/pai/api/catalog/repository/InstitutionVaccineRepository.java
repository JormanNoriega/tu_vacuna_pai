...
public interface InstitutionVaccineRepository extends JpaRepository<InstitutionVaccineEntity, UUID> {
    ...
    @Modifying
    @Query("update InstitutionVaccineEntity x set x.enabled=:enabled where x.id=:id and x.version=:version")
    int setEnabled(@Param("id") UUID id, @Param("version") long version, @Param("enabled") boolean enabled);
}
