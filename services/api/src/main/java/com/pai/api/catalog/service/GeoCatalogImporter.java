...
public class GeoCatalogImporter implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) throws Exception {
    ...
    GeoCountryEntity country = countries
        ...
        .orElseGet(() ->
            countries.save(new GeoCountryEntity(UUID.randomUUID(), COUNTRY_CODE, COUNTRY_NAME)));
    ...
    try (InputStream input = new ClassPathResource("catalog/divipola.json").getInputStream()) {
      for (JsonNode departmentNode : json.readTree(input)) {
        String departmentCode = departmentNode.path("code").asString();
        ...
        if (department == null) {
          department = new GeoDepartmentEntity(
              ...
              country.getId(),
              departmentCode,
              departmentNode.path("name").asString());
        ...
        for (JsonNode municipalityNode : departmentNode.path("municipalities")) {
          newMunicipalities.add(new GeoMunicipalityEntity(
              UUID.randomUUID(),
              department.getId(),
              municipalityNode.path("code").asString(),
              municipalityNode.path("name").asString()));
        }
      }
    }
    ...
    log.info(
        "Geo catalog seeded: {} departamentos, {} municipios",
        newDepartments.size(),
