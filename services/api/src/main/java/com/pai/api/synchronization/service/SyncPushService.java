...
public class SyncPushService {
    ...
    private void dispatch(UUID actorId, SyncOperation operation) {
        ...
        switch (operation.commandType()) {
            case CREATE_PATIENT ->
                patients.create(actorId, operationId, operation.aggregateId(), convert(operation, CreatePatientRequest.class));
            case CREATE_ATTENTION ->
                attentions.create(
                        actorId, operationId, operation.aggregateId(), convert(operation, CreateAttentionRequest.class));
            case REGISTER_APPLIED_DOSE ->
                attentions.registerDose(
                        actorId, operationId, attentionId(operation), operation.aggregateId(), convertDose(operation));
            case COMPLETE_ATTENTION -> attentions.complete(actorId, operationId, operation.aggregateId());
            default -> throw new IllegalArgumentException("Comando no soportado: " + operation.commandType());
        }
    }
    ...
    private String permissionFor(String commandType) {
        return switch (commandType) {
            ...
            case CREATE_ATTENTION, REGISTER_APPLIED_DOSE, COMPLETE_ATTENTION -> PERMISSION_ATTENTION_CREATE;
            default -> null;
        };
    }

    private void reject(
            List<RejectedOperation> rejected, Map<UUID, Boolean> outcomes, UUID operationId, String reason, String error) {
